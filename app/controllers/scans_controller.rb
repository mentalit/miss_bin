...
class ScansController < ApplicationController
  ...
  def capture
    ...
    result = CameraOcrService.scan_and_compare(article)

    # Always record mismatches automatically — no user action needed
    unless result[:match]
  ...
  rescue CameraOcrService::CaptureError => e
    render json: { error: "Camera error: #{e.message}" }, status: :service_unavailable
